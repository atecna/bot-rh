...
try {
  ...
  console.log(`Données chargées depuis ${CONFIG.dataPath} (${DATA_CONTENT.length} caractères)`);
} catch (error) {
...
export default async function askPholon(
  ...
  timer.log('Historique de conversation : ' + (conversationHistory?.length || 0) + ' messages');
  ...
  try {
    ...
    // log first 300 characters of DATA_CONTENT
    console.log(DATA_CONTENT.substring(0, 300));
    console.log('DATA_CONTENT.length', DATA_CONTENT.length);
    ...
    if (conversationHistory && conversationHistory.length > 0) {
      ...
      contents.push({
        role: 'assistant',
        parts: [{ text: "J'ai compris le contexte. Je vais maintenant répondre à vos questions en tenant compte de notre conversation précédente." }]
      });
